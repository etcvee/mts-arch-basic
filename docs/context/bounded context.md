...
Rel(admin, conference_mgmnt, "Использует", "Web")
Rel(admin, schedule_mgmnt, "Использует", "Web")
Rel(participant, online_streaming, "Использует", "Web")
Rel(participant, feedback_mgmnt, "Использует", "Web")
Rel(speaker, report_mgmnt, "Использует", "Web")
Rel(expert, report_mgmnt, "Использует", "Web")
Rel(report_mgmnt, email_system, "Использует для нотификаций", "SMTP")
